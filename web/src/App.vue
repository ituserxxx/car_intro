...
<template>
  <div class="relative min-h-screen overflow-hidden bg-background">
    <div class="pointer-events-none absolute inset-0 overflow-hidden">
      <div class="absolute -left-[10%] -top-[10%] h-[500px] w-[500px] rounded-full bg-primary/5 blur-[100px]" />
      <div class="absolute -bottom-[10%] -right-[10%] h-[600px] w-[600px] rounded-full bg-primary/5 blur-[120px]" />
    </div>

    <div class="relative z-10 mx-auto flex min-h-screen max-w-5xl flex-col px-6 py-16">
      <header class="mb-16 text-center">
        <div class="mb-6 inline-flex h-16 w-16 items-center justify-center rounded-2xl bg-primary text-primary-foreground shadow-lg">
        ...
        <h1 class="mb-3 text-4xl font-bold tracking-tight text-foreground sm:text-5xl">汽车说明书检索</h1>
        <p class="mx-auto max-w-lg text-lg text-muted-foreground">输入品牌关键词，快速定位用户手册与使用说明书</p>
      </header>

      <div class="mx-auto mb-12 w-full max-w-2xl space-y-4">
        <div class="flex items-center gap-3">
          <div class="group relative flex flex-1 items-center rounded-2xl border border-input bg-card shadow-xl shadow-black/5 transition-all duration-300 hover:shadow-2xl hover:shadow-black/10 focus-within:ring-2 focus-within:ring-ring">
            ...
            <input v-model="keyword" type="text" placeholder="例如：比亚迪、丰田、大众..." class="h-14 w-full bg-transparent px-4 text-base text-foreground placeholder:text-muted-foreground/70 outline-none" @keydown="onInput" />
          </div>

          <button @click="onSearch" :disabled="loading" class="inline-flex h-14 shrink-0 items-center justify-center rounded-2xl bg-primary px-8 text-base font-semibold text-primary-foreground shadow-lg shadow-primary/25 transition-all hover:bg-primary/90 hover:shadow-xl hover:shadow-primary/30 hover:-translate-y-0.5 active:translate-y-0 disabled:opacity-50 disabled:cursor-not-allowed">
        ...
        <div class="flex flex-wrap justify-center gap-2">
          <button v-for="tag in ['比亚迪', '丰田', '大众', '北京越野', '传祺', '长城']" :key="tag" @click="keyword = tag; onSearch()" class="rounded-full border border-border bg-secondary px-4 py-1.5 text-sm text-secondary-foreground transition-colors hover:bg-secondary/80">
            {{ tag }}
          </button>
        </div>
      </div>
      ...
      <div v-if="hasSearched" class="w-full">
        ...
        <div v-else-if="brands.length" class="grid gap-4">
          <div v-for="brand in brands" :key="brand" class="rounded-2xl border border-border bg-card shadow-sm overflow-hidden">
            <button @click="toggleBrand(brand)" class="flex w-full items-center justify-between p-5 text-left transition-colors hover:bg-muted/50">
              <div class="flex items-center gap-3">
                ...
                <span class="text-xl font-semibold text-foreground">{{ brand }}</span>
              </div>
            ...
            <div v-if="expanded.has(brand)" class="border-t border-border px-5 pb-5 pt-0">
              <div v-if="brandLoading[brand]" class="mt-4 space-y-3">
                <div v-for="i in 3" :key="i" class="h-12 animate-pulse rounded-xl bg-muted" />
              </div>
              <div v-else-if="brandFiles[brand] && brandFiles[brand].length" class="mt-4 space-y-5">
                <div v-for="group in brandFiles[brand]" :key="group.folder">
                  <h4 v-if="group.folder" class="mb-2 text-base font-semibold uppercase tracking-wider text-muted-foreground">{{ group.folder }}</h4>
                  <div class="grid gap-2 sm:grid-cols-2">
                    <div v-for="(item, idx) in group.files" :key="idx" @click="openPdf(brand, item.path)" class="flex cursor-pointer items-center gap-2 rounded-lg border border-border bg-background p-2.5 transition-all hover:border-primary/20 hover:shadow-sm">
                      ...
                      <p class="text-base font-medium text-foreground line-clamp-2">{{ item.name }}</p>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div v-else class="flex flex-col items-center justify-center rounded-2xl border border-dashed border-border bg-card py-16 text-center">
      ...
      <Teleport to="body">
        <div v-if="showPdfModal" class="fixed inset-0 z-50 flex flex-col bg-black/80 backdrop-blur-sm" @click.self="showPdfModal = false">
          <div class="flex items-center justify-between border-b border-border bg-card px-4 py-3">
            <span class="text-lg font-semibold text-foreground">PDF 预览</span>
      ...
      <footer class="mt-auto pt-16 text-center text-sm text-muted-foreground">
